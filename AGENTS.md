...
*   **Metadata Management:**
    ...
    *   *Example:*
        ...
        ```
    *   **Proxies & Serialization:** OBR uses `postMessage` to communicate. You **MUST** strip Vue reactivity (proxies) from objects before saving them to metadata.
        *   *Bad:* `OBR.room.setMetadata({ [KEY]: myRef.value })`
        *   *Good:* `OBR.room.setMetadata({ [KEY]: JSON.parse(JSON.stringify(myRef.value)) })`
*   **Reactivity:**
...
*   **"DataCloneError" / "Cannot clone object":**
    *   *Fix:* You are trying to send a Vue Proxy object to OBR. Use `JSON.parse(JSON.stringify(obj))` to strip reactivity before saving to metadata.
*   **"Lifecycle injection APIs can only be used during execution of setup()":**
    *   *Fix:* Ensure `onMounted`, `onUnmounted`, etc., are called synchronously in the top level of `setup()`. Do not call them inside `OBR.onReady` or other async callbacks.
*   **"Blocked autofocusing on a <input>":**
    *   *Fix:* Remove `autoFocus` attributes from inputs. Browsers block autofocus in cross-origin iframes (which extensions use) for security.
