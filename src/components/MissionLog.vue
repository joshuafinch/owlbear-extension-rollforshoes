...
<template>
    <div class="h-full flex flex-col bg-[var(--obr-surface-base)] border-t-4 border-[var(--obr-border-base)] relative">
        ...
        }" :character="characters.find(c => c.id === activeEvolutionEntry?.characterId)" :isRetroactive="true"
            :resultOverride="{
        ...
        <!-- Tape Header -->
        <div
            class="bg-[var(--obr-text-primary)] text-[var(--obr-text-inverse)] px-4 py-2 text-xs font-black uppercase tracking-widest flex justify-between items-center">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-[var(--obr-surface-base)]">
            ...
            <template v-for="entry in visibleHistory" :key="entry.id">
                <!-- ROLL ENTRY -->
                <div v-if="entry.type === LOG_TYPE_ROLL"
                    ...
                    <!-- Timestamp Badge -->
                    <div
                        class="absolute -right-2 -top-2 bg-[var(--obr-text-primary)] text-[var(--obr-text-inverse)] px-2 py-0.5 text-[10px] font-bold transform rotate-3 group-hover:rotate-0 transition-transform">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <div class="space-y-2">
                            ...
                            <template v-else>
                                <div class="flex flex-wrap items-center gap-2 justify-between pt-2">
                                    <div v-if="entry.isNpc"
                                        class="text-[10px] text-[var(--obr-text-disabled)] uppercase font-bold italic">
                                        <span>NPC Roll</span>
                                    </div>
                                    <div class="text-[10px] text-[var(--obr-text-disabled)] uppercase font-bold italic">
                                        <!-- NPC Roll, No Actions Available -->
                                    </div>
                ...
                <!-- SKILL ENTRY -->
                <div v-else-if="entry.type === LOG_TYPE_SKILL"
                    ...
                    <!-- Achievement Block -->
                    <div
                        class="bg-[var(--obr-primary-main)]/5 rounded p-3 text-center border border-[var(--obr-primary-main)]/20 mb-2 relative overflow-hidden">
                        <!-- Decorator Icon -->
                        <div
                            class="absolute -right-4 -bottom-4 text-[var(--obr-primary-main)] opacity-10 text-6xl transform rotate-12 select-none">
                            ★</div>

                        <div
                            class="text-[9px] uppercase text-[var(--obr-primary-main)] tracking-[0.2em] font-black mb-1">
                            ✨ Skill Acquired
                        </div>
                        ...
                        <div
                            class="inline-block bg-[var(--obr-text-primary)] text-[var(--obr-text-inverse)] text-[10px] font-bold px-2 py-0.5 rounded shadow-sm mt-1 relative z-10">
                            Rank {{ entry.rank }}
                        </div>
                    </div>
